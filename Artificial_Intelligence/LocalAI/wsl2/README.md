...
Timing updates come from the response stream using llama.cpp `timings_per_token` and `return_progress`. The terminal refreshes at 100 ms by default (configurable from 50 to 250 ms using `LLAMA_LIVE_REFRESH_SECONDS`). Millisecond units do not imply one new measurement per millisecond: a 1,000-Hz redraw would consume resources without making inference faster. Once semantic output arrives, redundant server-slot polling stops. A final speed summary remains visible after each model request. Every working progress row retains a token-speed indicator. Tool operations show `0 T/m (model idle)` alongside measured files/second (average since process start), time since the last checkpoint, and filesystem counts; deterministic answers explicitly report that token speed is not applicable because the model was not used.
